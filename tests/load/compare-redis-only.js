// File: tests/load/compare-redis-only.js
// Comparison test for Redis-only (no Kafka) implementation
// Tests: No race conditions, but synchronous DB writes bottleneck
...
export const options = {
  ...
  thresholds: {
    http_req_duration: ['p(95)<800', 'p(99)<1500'],  // Slower than optimized due to DB writes
    errors: ['rate<0.1'],
  },
};
...
export default function (data) {
  ...
  const idempotencyKey = `compare-redis-${userId}-${Date.now()}-${Math.random()}`;
  ...
  const params = {
    ...
    timeout: '15s',  // Longer timeout for synchronous DB writes
  };
...
export function handleSummary(data) {
  return {
    'tests/load/results/compare-redis-only.json': JSON.stringify(data, null, 2),
